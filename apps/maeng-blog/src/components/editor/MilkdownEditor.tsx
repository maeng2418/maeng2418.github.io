...
// 이미지 삽입은 M3/M4 업로드 Route Handler(/api/images)와 연동한다(REQ-EDITOR-006).
...
function EditorCore({ defaultValue, onChange }: MilkdownEditorProps) {
  ...
  // 이미지 업로드 → /content-images/{unique}.{ext} 삽입 (클라이언트는 ky/fetch 만 사용 — SDK 미포함, REQ-EDITOR-010)
  const uploadAndInsertImage = async (file: File) => {
    ...
    try {
      ...
      get()?.action(callCommand(insertImageCommand.key, { src: json.path, alt }))
    } catch (error) {
