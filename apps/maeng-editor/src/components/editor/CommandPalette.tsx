...
// 커맨드 팔레트 — ⌘K/Ctrl+K (REQ-ENH-011, design.md §5)
...
// 캐시를 소비하고(부모가 props 로 공급), 액션(저장/동기화/로케일 전환)을 함께 노출한다.
// 문자열은 전부 next-intl ko/en 카탈로그 경유 (REQ-ENH-004 승계).
...
export interface CommandPaletteProps {
  ...
  onSave: () => void
  /** capability gate — blogSyncEnabled 일 때만 공급 (미공급 시 항목 미노출) */
  onSync?: () => void
  onToggleLocale: () => void
}
...
export default function CommandPalette({
  ...
  // 열릴 때 입력에 포커스 — Esc 닫힘 시 에디터 포커스는 브라우저 기본 복원에 맡긴다 (GWT-3)
  useEffect(() => {
