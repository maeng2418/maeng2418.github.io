// AC-EDITOR-003 — 블로그 콘텐츠 계약 라운드트립 (TDD RED→GREEN)
...
// maeng-blog 픽스처 디렉터리 (read-only — REQ-EDITOR-013: 블로그 앱 무변경)
const BLOG_MARKDOWNS_DIR = path.resolve(__dirname, '../../../../../maeng-blog/content/markdowns')
...
describe('serializePostMarkdown — 정확히 5-키 계약 (REQ-EDITOR-003)', () => {
  const fm = {
    title: '테스트 포스트',
...
describe('parsePostMarkdown ↔ serializePostMarkdown 라운드트립', () => {
...
describe('blog 픽스처 교차 검증 (AC-EDITOR-003 verbatim)', () => {
...
describe('markdowns/{category}/ 키 레이아웃 헬퍼 (legacy maeng-bucket 구조)', () => {
  it('buildMarkdownKey 가 legacy 레이아웃 키를 생성한다', () => {
  ...
  it('prefix 를 앞에 결합한다 (env AWS_BUCKET_MARKDOWN_FOLDER_PREFIX 대응)', () => {
    expect(buildMarkdownKey('dev', 'a', 'bucket-prefix')).toBe('bucket-prefix/markdowns/dev/a.md')
    expect(buildMarkdownKey('dev', 'a', 'bucket-prefix/')).toBe('bucket-prefix/markdowns/dev/a.md')
  })

  it('경로 조작 문자(../, 슬래시)를 거부한다', () => {
